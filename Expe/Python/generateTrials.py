...
def genTrialList():
    ...
    neutStim = [stim for stim in stimList if neg(stim)]
    negStim = [stim for stim in stimList if neut(stim)]
    ...
    for run in range(4):
        ...
        writeTrials = csv.writer(open('run' + str(run) + '.csv','wb'), delimiter = ',', quotechar = '"')
       ...
       # for i in range(1,len(trials), len(trials)/2):
        ...
        for i in range(len(trials)):
            if trials[i] == 3:
                ...
                stim = Neg_run[indNeg]
                trial = [stim,'neg',str(ISI[i])]
                indNeg += 1
            elif trials[i] == 6: 	
                stim = Neut_run[indNeut]
                trial = [stim,'neut',str(ISI[i])]
                indNeut += 1
