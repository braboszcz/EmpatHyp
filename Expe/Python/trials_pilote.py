...
def genTrialList():
	...
	header = ['Stim', 'Condition', 'ITI', 'Painful', 'Negative']
	...
	#------------------------------------------------------------------------
	nNoGo = 30*4
	nGo = 32 #(  20% de nNoGo. allows for 8 pictures of each conditions)
#	nOdd = 5	
...
#	trials += [ODDBALL] * nOdd
	...
	for i in range(len(trials)):
		if trials[i] == 3:
			...
			trial = [stim,'no-go',str(ISI[i]), 1 if '_P_' in stim else 0, 1 if '_E_' in stim else 0]
			indnoGo += 1
		#	print indnoGo
		elif trials[i] == 1:	
			stim = goStim[indGo]
			
			trial = [stim,'go',str(ISI[i]), 1 if '_P_' in stim else 0, 1 if '_E_' in stim else 0]
			indGo += 1
		#	print indGo
