...
temperatures = [40, 42] #... initial
n_turn=2
n_stair = 2
count_turn = 0
def check_resp(resp, critical_res):

    if resp >= critical_res :
        is_critic = False
    else
        is_critic = True
    return(is_critic)

def get_temp(temp,is_critic,resp, prev_resp):
...
while count_turn > n_turn*n_stair:
    ...
    temp, count_turn = get_next_temp(temperatures[idx], resp)
